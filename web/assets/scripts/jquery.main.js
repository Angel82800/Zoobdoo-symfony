 ...
 * @returns {undefined}
 */
function processResponse(noAjax) {
    //check if we have form in fancybox
    var form = $(document).find('.fancybox-inner form');
    if (form && !noAjax) {
...
function initSlickCarousel() {
    ResponsiveHelper.addRange({
        '..767': {
            ...
            off: function () {
                jQuery('.nav-slider').slick('destroy');
            }
        }
    });
...
function initMobileNav() {
    jQuery('body').mobileNav({
        menuActiveClass: 'setting-active',
        menuOpener: '.setting-opener',
        hideOnClickOutside: true,
        menuDrop: '.setting-slide'
    });
    jQuery('body').mobileNav({
        menuActiveClass: 'message-active',
        menuOpener: '.message-opener',
        hideOnClickOutside: true,
        menuDrop: '.message-slide'
    });
    jQuery('body').mobileNav({
        menuActiveClass: 'profile-active',
        menuOpener: '.profile-opener',
        hideOnClickOutside: true,
        menuDrop: '.profile-slide'
    });
}
...
function initLightbox() {
    ...
    jQuery('a.lightbox-ajax, a[rel*="lightbox-ajax"]').click(function (event) {
        event.preventDefault();
        var $this = $(this), link = $this.attr('href'), method = ($this.attr('method') || 'GET');
