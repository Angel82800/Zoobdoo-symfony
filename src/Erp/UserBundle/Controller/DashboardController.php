...
class DashboardController extends BaseController {
    ...
    public function showCashflowsAction() {
        ...
        if ($stripeAccount || $stripeCustomer) {
            $cashOut = $this->getPreparedItems($transactionRepo->getGroupedTransactions(null, $stripeCustomer, $sixMonthsAgo, $now), $intervals);
        }

        $cashIn = [];
        if ($stripeAccount) {
            $cashIn = $this->getPreparedItems($transactionRepo->getGroupedTransactions($stripeAccount, null, $sixMonthsAgo, $now), $intervals);
        }
    ...
    public function showInvoicesAction() {
        ...
        $items = [];
        if ($stripeAccount || $stripeCustomer) {
            $invoicesRepo = $this->em->getRepository(Invoice::class);
