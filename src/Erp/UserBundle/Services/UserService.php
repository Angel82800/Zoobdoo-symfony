...
class UserService {
    ...
    public function createPaySimpleCustomer(User $user) {
        ...
        $paySimpleManager = PaySimpleManagerFactory::getManagerInstance(
                        PaySimpleManagerInterface::TYPE_CUSTOMER, $this->container, $paySimpleCredentials['paySimpleLogin'], $paySimpleCredentials['paySimpleApiSecretKey']
        );
    ...
    public function createPaySimplePayment(PaySimpleModelInterface $model, $type) {
        ...
        $paySimpleManager = PaySimpleManagerFactory::getManagerInstance(
                        PaySimpleManagerInterface::TYPE_PAYMENT, $this->container, $paySimpleCredentials['paySimpleLogin'], $paySimpleCredentials['paySimpleApiSecretKey']
        );

        $method = $type === PaySimpleManagerInterface::CREDIT_CARD ? PaySimpleManagerInterface::METHOD_PAYMENT_CREATE_CC : PaySimpleManagerInterface::METHOD_PAYMENT_CREATE_BA;
        ...
        if ($response['status'] === PaySimpleManagerInterface::STATUS_OK) {
            ...
            if (!$response['data']['IsDefault']) {
                $paySimpleManager = PaySimpleManagerFactory::getManagerInstance(
                                PaySimpleManagerInterface::TYPE_PAYMENT, $this->container
                );
    ...
    public function createPaySimplePaymentRecurring(RecurringPaymentModel $model) {
        ...
        if ((!$isLastRecurrring || ($isLastRecurrring && $lastRecurringResponse['status'])) && $paymentResponse['status']
        ) {
            $paySimpleCredentials = $this->getCurrentPaySimpleCredentials($model->getCustomer()->getUser());

            $paySimpleManager = PaySimpleManagerFactory::getManagerInstance(
                            PaySimpleManagerInterface::TYPE_RECURRING, $this->container, $paySimpleCredentials['paySimpleLogin'], $paySimpleCredentials['paySimpleApiSecretKey']
            );
            ...
            if ($response['status'] === PaySimpleManagerInterface::STATUS_OK) {
                ...
                if ($model->getCustomer()->getUser()->hasRole(User::ROLE_TENANT) && $model->getStartDate()->format('Y-m-d') == (new \DateTime())->format('Y-m-d')
                ) {
            ...
            $result['status'] = false;
            $result['errors'] = (isset($lastRecurringResponse['errors'])) ? $lastRecurringResponse['errors'] : [['Message' => $this->getPaySimpleErrorByCode('one_check_payment_error')]];
        }
    ...
    public function makeOnePayment(RecurringPaymentModel $model, PaySimpleDeferredPayments $deferredPayment = null) {
        ...
        if ($model->getStartDate()->format('Y-m-d') === (new \DateTime())->format('Y-m-d')) {
            ...
            if ($response['status'] === PaySimpleManagerInterface::STATUS_OK && (
                    $response['data']['Status'] == PaySimpleDeferredPayments::STATUS_AUTHORIZED || $response['data']['Status'] == PaySimpleDeferredPayments::STATUS_POSTED
                    )
            ) {
                if (!$deferredPayment) {
                ...
                if ($model->getCustomer()->getUser()->hasRole(User::ROLE_TENANT)) {
                    ...
                    $status = $statusCC || $statusBA ?
                            PaySimpleHistory::STATUS_SUCCESS : PaySimpleHistory::STATUS_ERROR;
    ...
    public function makeOnePaymentAnonymous(User $user, $data) {
        ...
        if ($result['status']) {
            ...
            if ($result['status']) {
                ...
                $this->logger->add(
                        'paysimple', 'PaySimple Payment Anonymous Response => ' . json_encode($response), 'INFO'
                );

                $result['status'] = true;
                if ($response['status'] == PaySimpleManagerInterface::STATUS_OK && $response['data']['Status'] == PaySimpleDeferredPayments::STATUS_AUTHORIZED
                ) {
                    $result['psPaymentMake'] = $response['data'];
                } else {
                    $result['status'] = false;
                    $result['errors'] = isset($response['errors']) ? $response['errors'] : [0 => ['Message' => $this->getPaySimpleErrorByCode('error')]];
                }
            }
        }
    ...
    public function addHistoryPayment(RecurringPaymentModel $recurringPaymentModel, $status, $isWithAllowance = false) {
        ...
        if ($isTenant && $paySimpleCredentials['paySimpleLogin'] && $paySimpleCredentials['paySimpleApiSecretKey'] && $status == PaySimpleHistory::STATUS_SUCCESS && $recurringPaymentModel->getAllowance() > 0
        ) {
    ...
    public function getCustomerPaymentAccountInfo(User $user, $type) {
        ...
        if ($userPSCustomer) {
            ...
            if ($result['data']) {
                $customerPayId = $userPSCustomer->getPrimaryType() === PaySimpleManagerInterface::CREDIT_CARD ? $userPSCustomer->getCcId() : $userPSCustomer->getBaId();
    ...
    public function getCustomerInfo(User $user) {
        ...
        if ($userPSCustomer) {
            ...
            if (!$result['data']) {
                ...
                $paySimpleManager = PaySimpleManagerFactory::getManagerInstance(
                                PaySimpleManagerInterface::TYPE_CUSTOMER, $this->container
                );
                $response = $paySimpleManager->setModel($model)->proccess(
    ...
    public function validateCredentials($paySimpleLogin, $paySimpleApiSecretKey) {
        $paySimpleManager = PaySimpleManagerFactory::getManagerInstance(
                        PaySimpleManagerInterface::TYPE_CUSTOMER, $this->container, $paySimpleLogin, $paySimpleApiSecretKey
        );
