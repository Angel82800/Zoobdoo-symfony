...
class Managers extends BaseAdmin {
        ...
        if ($object instanceof UserInterface) {
            ...
            if (!$result) {
                ...
                $this->getRequest()->getSession()->getFlashBag()->add(
                        'sonata_flash_ps_error', 'Private Payment Credentials were not saved'
                );
            }
        }
    ...
    protected function configureFormFields(FormMapper $formMapper) {
        ...
        if (!$isStatusDisabled and $user->getStatus() == User::STATUS_ACTIVE || !count($user->getPaySimpleCustomers())
        ) {
...
//            }
        ...
        $this->formMapper->add('isPrivatePaySimple', null, [
            'disabled' => $issetTenants,
            'label' => 'Enable Private account'
            . (($issetTenants) ? ' (Action is not allowed, because Manager has tenant(-s))' : ''),
        ]);
        $this->formMapper->add('paySimpleLogin', null, [
            'label' => 'Username',
            'disabled' => $issetTenants,
        ]);
        $this->formMapper->add('paySimpleApiSecretKey', 'text', [
